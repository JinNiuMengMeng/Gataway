...
//	static uint8_t secret[32];
	...
	struct tm *lt;
	ii = time(&t);
	j = sprintf(stamp, "%d", ii);

	len = strlen(msg);
	md5((uint8_t*)msg, len, result);

	memcpy(secret, stamp, sizeof(stamp)/sizeof(uint8_t));
	for(i = 0; i < 7; i++) {
		sprintf(secret+10+2*i, "%02x", result[i]);
	}
